...
const getPokemons = async (req, res) => {
    try {
        // accedo a la api, primero al objeto y despues a su propiedad url que es un link y tiene toda la data
        ...
        if(name){
            ...
            if(getByName){
                res.json(getByName)
                    ...
                    res.send({msg: "Pokemon not found"})
                } 
            } else {
...
const createPokemon = async (req, res) => {
    ...
    try {
      ...
      /*   types.map(async t=>{
            const typesDb = await Types.findOne({
                where: { id: t}
            })
        }) */
